...
### 三、日主性格分析
分析内容要点：日干天性（如丙火热情、壬水变通等）；十大神中哪些最突出（如伤官旺则聪明心高、印旺则稳重依赖等）；五行旺气对性格的影响（火旺性急、土旺稳重等）；关键刑冲合害对性格的影响

### 四、婚姻分析
分析内容要点：女命看正官/七杀（夫星），男命看正财/偏财（妻星）——夫妻星是否在原局出现、位置在哪（年柱早婚、时柱晚婚）、是否透干；日支夫妻宫与其他地支的刑冲合害关系（合多则情易分、冲多则矛盾大）；是否官杀混杂（女命不利）；是否伤官克官（女命大忌）；是否在24个好日柱中；女命出轨判断（禄在日/时支且合他支）
...
def _call_deepseek_api(prompt: str, api_key: str) -> Optional[dict]:
    ...
    payload = json.dumps({
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": "你是八字命理专家，严格按以下顺序分析：①调候（健康）②格局（事业财富）③日主性格分析④婚姻分析⑤大运⑥结论。核心原则：调候和格局同时分析不冲突；不要死抓日主强弱，八字平衡看全局五行旺衰；不需日主时不管日主强弱；无平衡之法则命不好。用中文，说人话。"},
